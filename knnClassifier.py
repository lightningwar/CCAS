...
def cal_best_K(vect, X_train, y_train):
    k_range = range(1, 21)
    cv_scores = []
    for n in k_range:
    ...
    pic_name = 'knn_bestk_' + time_name + '.png'
    plt.savefig(f'static/Classifier/knn_img/' + pic_name, dpi=100, bbox_inches='tight')
    plt.close()
    return best_K, best_cross_score, pic_name


def model_eva(vect, best_K):
...
　　 """
    ...
    time_name = str(time.strftime('%Y%m%d%H%M%S', time.localtime(time.time())))
    pic_name = 'knn_matrix_' + time_name + '.png'
    plt.savefig(f'static/Classifier/knn_img/' + pic_name, dpi=100, bbox_inches='tight')
    plt.close()
    print('d')
    return pic_name
...
def ROC(pipe, X_test, y_test):
    ...
    # y_score = knn.predict_proba(vect.fit_transform(X1_test.cut_jieba).toarray())[:, 1]
    ...
    time_name = str(time.strftime('%Y%m%d%H%M%S', time.localtime(time.time())))
    pic_name = 'knn_roc_' + time_name + '.png'
    plt.savefig(f'static/Classifier/knn_img/' + pic_name, dpi=100, bbox_inches='tight')
    plt.close()
...
def pr(y_test, y_score):
    ...
    time_name = str(time.strftime('%Y%m%d%H%M%S', time.localtime(time.time())))
    pic_name = 'knn_pr_' + time_name + '.png'
    plt.savefig(f'static/Classifier/knn_img/' + pic_name, dpi=100, bbox_inches='tight')
    plt.close()
    print('g')
    return pic_name
